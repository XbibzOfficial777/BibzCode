...
describe('workspace path boundary', () => {
  ...
  it('does not inherit provider secrets into child processes', () => {
    const previous = process.env.BIBZCODE_API_KEY;
    process.env.BIBZCODE_API_KEY = 'test-secret-that-must-not-propagate';
    try {
      const child = safeChildEnvironment({ BIBZCODE_WORKSPACE: '/workspace' });
      expect(child.BIBZCODE_API_KEY).toBeUndefined();
      expect(child.BIBZCODE_WORKSPACE).toBe('/workspace');
    } finally {
      if (previous === undefined) delete process.env.BIBZCODE_API_KEY;
      else process.env.BIBZCODE_API_KEY = previous;
    }
  });
