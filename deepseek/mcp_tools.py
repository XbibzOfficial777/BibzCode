...
# ═══════════════════════════════════════════════════════════
# ERROR HANDLING & VALIDATION (Safe Wrappers)
# ═══════════════════════════════════════════════════════════

class SafeJSONRPCMessage(dict):
    """Safe JSONRPC message wrapper - never crashes on validation."""
    def __init__(self, data):
        try:
            if isinstance(data, dict):
                super().__init__(data)
            else:
                super().__init__({'raw': str(data)[:100]})
        except:
            super().__init__({'error': 'Invalid message'})
...
def safe_parse_json(text):
    ...
    try:
        ...
        return {'error': 'Parse failed', 'raw': text[:100]}
