...
#   Agnes AI via the OpenAI-compatible endpoint with native streaming tool_calls)
#
# Architecture:
...
class OpenAICompatibleProvider(BaseProvider):
    ...
    def chat_stream(self, messages: list, model: str = None,
        ...
        # Sanitize tool call arguments in messages to prevent API 400 errors
        _sanitized_messages = []
        for msg in messages:
                ...
                msg['tool_calls'] = [
                    {
                        **tc,
                        'function': {
                            **tc.get('function', {}),
                            'arguments': self._sanitize_json(tc.get('function', {}).get('arguments', '{}'))
                        }
                    }
                    for tc in msg['tool_calls']
                ]
            _sanitized_messages.append(msg)
