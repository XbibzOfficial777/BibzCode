...
# (banned account, quota exceeded, Ctrl-C) via non-zero exit codes, and with
# `set -e` the wrapper aborted before those messages could be seen.
set -uo pipefail
...
# If stdin is not a terminal (piped script, cron, etc.) re-attach the real
# controlling terminal when one exists, so the REPL does not read EOF and
# quit instantly with no output.
if [ ! -t 0 ] && [ -e /dev/tty ]; then
    exec "\$PYTHON" -m deepseek "\$@" </dev/tty
fi
...
# With `curl ... | bash`, stdin is the *script text*, so exec'ing dscli here
# handed the REPL an already-consumed stdin: it hit EOF instantly and exited
# without printing anything. Require a real /dev/tty and re-attach it.
if [ $# -eq 0 ] && [ -e /dev/tty ] && tty -s 0</dev/tty 2>/dev/null; then
    echo -en "${CY}${B}  Launch DeepSeek CLI now? [Y/n]${R} "
    read -r ANSWER </dev/tty 2>/dev/null || ANSWER="y"
    case "$ANSWER" in
        n*|N*) ;;
        # Re-attach the controlling terminal so the REPL gets a real stdin.
        *) echo ""; exec dscli </dev/tty ;;
    esac
elif [ $# -eq 0 ]; then
    echo -e "${CY}${B}  Run ${R}${B}dscli${R}${CY}${B} to start.${R}"
fi